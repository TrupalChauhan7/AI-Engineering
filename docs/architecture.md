...
Pipeline: **audio → Whisper → transcript → LLM → SOAP note → evaluation → ALARM**
...
| alarm | reliability flag (the contribution) |
| llm | backend-agnostic model access |

## Validation logic (why anyone should trust the alarm)
Auto faithfulness score → correlate with PriMock57 human ratings → report
flagging precision/recall. If the score tracks humans, the alarm is trustworthy.
